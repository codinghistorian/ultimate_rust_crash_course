...
fn main() {
    ...
    // loop through several values and print each out and then send it to the channel

    let (tx0, rx0) = channel::unbounded::<i32>();
    ...
    let handle_c = thread::spawn(move || {
        println!("Handle C is printing{}", rx0.recv().unwrap());
    });


    let handle_d = thread::spawn(move || {
        println!("Handle D is printing{}", rx9.recv().unwrap());
    });
